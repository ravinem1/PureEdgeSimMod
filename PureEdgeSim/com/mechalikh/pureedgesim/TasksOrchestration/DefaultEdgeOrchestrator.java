...
public class DefaultEdgeOrchestrator extends Orchestrator {
	...
	protected int[] findVM(String[] architecture, List<Task> tasks) {
		//for (String task : tasks) {
		
		if ("ROUND_ROBIN".equals(algorithm)) {
			return roundRobin(architecture, tasks);
		} else if ("TRADE_OFF".equals(algorithm)) {
			return tradeOff(architecture, tasks);
		} else {
			SimLog.println("");
	...
	private int[] roundRobin(String[] architecture, List<Task> tasks) {
		int[] result = new int[tasks.size()];
		int j=0;
		for (Task task : tasks) {		
