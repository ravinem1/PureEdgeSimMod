...
import com.mechalikh.pureedgesim.Helper.Helper;
...
public class SimulationManager extends SimulationManagerAbstract {
	...
	public void startEntity() {
		...
		simLog.setGeneratedTasks(Helper.GetTotalTasksCount(tasksList));
	...
	private void sendFromOrchToDestination(Task task) {
		...
		// failed
		List<Task> tasks = Helper.GetFlattenedTasks(task);
		for (Task t : tasks) {
			if (t.getVm() == Vm.NULL) {
				simLog.incrementTasksFailedLackOfRessources(t);
				tasksCount++;
				return;
			} else {
				simLog.taskSentFromOrchToDest(t);
			}
			// If the task is offloaded
		// and the orchestrator is not the offloading destination
		if(t.getOrchestrator() == null)
		{
			t.setOrchestrator(task.getOrchestrator());
		}
			if (t.getEdgeDevice().getId() != t.getVm().getHost().getDatacenter().getId()
				&& t.getOrchestrator() != ((DataCenter) t.getVm().getHost().getDatacenter())) {
					scheduleNow(networkModel, NetworkModelAbstract.SEND_REQUEST_FROM_ORCH_TO_DESTINATION, t);
				}
			else 
			{ // The task will be executed locally / no offloading or will be executed where
	...
	private void sendTaskToOrchestrator(Task task) {
		task = taskActive(task, 0);
		if (task == null)
			return;

		List<Task> allTasks = Helper.GetFlattenedTasks(task);
		simLog.incrementTasksSent(allTasks.size());
		...
		if (SimulationParameters.ENABLE_ORCHESTRATORS) {
			...
			if (orchestratorsList.size() == 0) {
				...
				tasksCount = tasksCount + allTasks.size();
				return;
			}
			for (Task task2 : allTasks) {
				task2.setOrchestrator(orchestratorsList.get(selected));
			}
	...
	public double getFailureRate() {
		double result = (failedTasksCount * 100) / Helper.GetTotalTasksCount(tasksList);
		failedTasksCount = 0;
		return result;
	}

	public Task taskActive(Task task, int phase) {
		
		List<Task> allActiveTasks = new ArrayList<Task>();
		...
		else if (phase == 0 && task.getEdgeDevice().isDead()) {
		...
		else if (phase == 1 && task.getOrchestrator() != null && task.getOrchestrator().isDead()) {
			task.setFailureReason(Task.Status.FAILED_BECAUSE_DEVICE_DEAD);
			simLog.incrementFailedBeacauseDeviceDead(task);
			setFailed(task);
		}
		// or the destination device is dead
		else if (phase == 2 && ((DataCenter) task.getVm().getHost().getDatacenter()).isDead()) {
			task.setFailureReason(Task.Status.FAILED_BECAUSE_DEVICE_DEAD);
			simLog.incrementFailedBeacauseDeviceDead(task);
			setFailed(task);
		}
		...
		else if (phase == 1 && task.getOrchestrator() != null
		...
		else if (phase == 2 && (task.getVm().getHost().getDatacenter()) != null
		...
		task = task.getNextInLineTask();
		}
		if(allActiveTasks == null || allActiveTasks.isEmpty())
		{
			return null;
		}
		return Helper.GetLinkedTasks(allActiveTasks);
	}
