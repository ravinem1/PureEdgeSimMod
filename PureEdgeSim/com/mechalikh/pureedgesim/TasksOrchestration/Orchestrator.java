...
public abstract class Orchestrator {
	...
	private void cloudOnly(List<Task> task) {
	...
	private void edgeAndCloud(List<Task> task) {
	...
	private void mistAndCloud(List<Task> task) {
	...
	// centers
	private void edgeOnly(List<Task> task) {
	...
	// or device
	private void all(List<Task> task) {
		String[] Architecture = { "Cloud", "Edge", "Mist" };
		assignTaskToVm(findVM(Architecture, task), task);
	}

	protected abstract int[] findVM(String[] architecture, List<Task> task);
 

	protected void assignTaskToVm(int[] vmIndexes, List<Task> tasks) {
		int i=0;
		for (Task task : tasks) {
			int vmIndex = vmIndexes[i];
		if (vmIndex == -1) {
